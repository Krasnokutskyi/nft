...
@section('content')
            ...
            <div class="card-body">
              <form method="POST" preloader-ajax-form=".card" class="ajax-form custom-form" enctype="multipart/form-data">
                ...
                <div class="form-group has-validation">
                  ...
                  <input class="form-control" type="text" id="title-input" name="title" autocomplete="off" required="required" value="{{ $file->title }}">
                ...
                <div class="form-group">
                  ...
                  <select name="file_type" id="type-input" class="form-control form-control-lg" autocomplete="off" required>
                    @foreach ($types as $type)
                      <option @if ($type->id === $file->type_id) selected @endif value="{{ $type->title }}">{{ $type->title }}</option>
                    @endforeach
                  </select>
                ...
                <div class="form-group">
                  ...
                  <select name="access" id="access-input" class="form-control form-control-lg" autocomplete="off" required>
                    <option @if($file->access === 'all') selected @endif value="all">All</option>
                    @if ($packages->count() > 0)
                      <option @if($file->access === 'packages') selected @endif value="packages">By package</option>
                    @endif
                    <option @if($file->access === 'nobody') selected @endif value="nobody">Nobody</option>
                  </select>
                    ...
                    <div class="checkboxradio-group">
                      @foreach ($packages as $package)
                        <div class="form-check">
                          <input @if($file->packages->where('id', '=', $package->id)->count() > 0) checked @endif  type="checkbox" name="packages[]" value="{{ $package->id }}" id="{{ $package->id }}-package-checkbox" class="form-check-input" autocomplete="off">
                ...
                <div class="form-group">
                  ...
                  <input type="text" name="remote_file" hidden="hidden" autocomplete="off">
  ...
  {{ HTML::script('admin/js/content/downloads/edit_file.js') }}
  {{ HTML::script('admin/js/ajax-form.js') }}

  <script type="text/javascript">
    var preview = {!! json_encode($file->preview) !!};
